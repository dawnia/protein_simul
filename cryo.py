...
def estimate_orientations2(images):
    ...
    #likewise l_21 is the angle from the positive x axis in the im2 local coordiantes where L12 is.
    ...
    #rotates about the x-axis(wher l_21 is aligned), by alpha
    ...
    F3 = np.column_stack((V13, V23, np.cross(V13, V23))) @ np.linalg.inv(np.column_stack((l_31_to_move, l_32_to_move, np.cross(l_31_to_move, l_32_to_move))))
